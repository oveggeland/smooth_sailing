...
if __name__ == "__main__":
    ...
    source_path = os.path.join(exp_path, "raw_clouds")
    ...
    for cloud_file in os.listdir(source_path):
        pcd = o3d.t.io.read_point_cloud(os.path.join(source_path, cloud_file))
        ...
        print(f"Applying topo threshold ({-3}, {3})")
        pcd = apply_filter(pcd, lambda x: t_filter(x, channel="topo", min=-3, max=3))
        
        # Only keep certain attributes
        new_point_cloud = o3d.t.geometry.PointCloud()
        new_point_cloud.point["positions"] = pcd.point["positions"]
        new_point_cloud.point["timestamps"] = pcd.point["timestamps"]
        pcd = new_point_cloud
